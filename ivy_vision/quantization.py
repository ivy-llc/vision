...
def quantize_to_image(pixel_coords, final_image_dims, feat=None, feat_prior=None, with_db=False,
    ...
    if dev_str is None:
        dev_str = _ivy.dev(pixel_coords)

    if feat is None:
        d = 0
    else:
        d = feat.shape[-1]
    min_depth_diff = _ivy.array([MIN_DEPTH_DIFF], device=dev_str)
    ...
    if isinstance(var_threshold, tuple) or isinstance(var_threshold, list):
        ...
        var_threshold = _ivy.concat((ones * var_threshold[0], ones * var_threshold[1]), axis=-1)
    else:
    ...
    # BS x N x 2
    quantized_pixel_coords = _ivy.reshape(_ivy.astype(_ivy.round(pixel_coords), 'int32'), batch_shape + [-1, 2])

    # Combine #

    # BS x N x (2+D)
    pc_n_feat = _ivy.reshape(_ivy.concat((pixel_coords, feat), axis=-1), batch_shape + [-1, 2+d])
    pc_n_feat_var = _ivy.reshape(_ivy.concat((pixel_coords_var, feat_var), axis=-1), batch_shape + [-1, 2+d])

    # BS x H x W x (2+D)
    prior = _ivy.concat((uniform_pixel_coords, feat_prior), axis=-1)
    prior_var = _ivy.concat((pixel_coords_prior_var, feat_prior_var), axis=-1)

    # Validity Mask #

    # BS x N x 1
    var_validity_mask = \
        _ivy.sum(_ivy.astype(pc_n_feat_var < var_threshold[..., 1], 'int32'), axis=-1, keepdims=True) == 2+d
    ...
    validity_indices = _ivy.reshape(_ivy.astype(_ivy.indices_where(validity_mask), 'int32'), [-1, num_batch_dims + 2])
    num_valid_indices = validity_indices.shape[-2]

    if num_valid_indices == 0:
        return _ivy.concat((uniform_pixel_coords[..., 0:2], feat_prior), axis=-1), \
               _ivy.concat((pixel_coords_prior_var, feat_prior_var), axis=-1),\
               _ivy.zeros_like(feat[..., 0:1], device=dev_str)
    ...
    if with_db:
        ...
        # BS x 1 x 1
        mean_depth_min = _ivy.min(mean_depth, axis=-2, keepdims=True)
        mean_depth_max = _ivy.max(mean_depth, axis=-2, keepdims=True)
        ...
        if d == 1:
            ...
            # BS x 1 x 1+D
            pc_n_feat_wo_depth_min = _ivy.zeros(batch_shape + [1, 0], device=dev_str)
            pc_n_feat_wo_depth_range = _ivy.ones(batch_shape + [1, 0], device=dev_str)

        else:
            # feat without depth

            # BS x N x 1+D
            pc_n_feat_wo_depth = _ivy.concat((pc_n_feat[..., 0:2], pc_n_feat[..., 3:]), axis=-1)

            # find the min and max of each value

            # BS x 1 x 1+D
            pc_n_feat_wo_depth_max = _ivy.max(pc_n_feat_wo_depth, axis=-2, keepdims=True) + 1
            pc_n_feat_wo_depth_min = _ivy.min(pc_n_feat_wo_depth, axis=-2, keepdims=True) - 1
        ...
        # BS x 1 x (2+D)
        var_vals_max = _ivy.max(pc_n_feat_var, axis=-2, keepdims=True) + 1
        var_vals_min = _ivy.min(pc_n_feat_var, axis=-2, keepdims=True) - 1
        ...
        # BS x 1 x 1 x D
        var_vals_min = _ivy.expand_dims(var_vals_min, axis=-2)
        var_vals_range = _ivy.expand_dims(var_vals_range, axis=-2)
    ...
    values_to_scatter = _ivy.concat((means_to_scatter, vars_to_scatter, counter), axis=-1)

    # num_valid_indices x (num_batch_dims + 2)
    all_indices = _ivy.flip(quantized_pixel_coords, axis=-1)
    if num_batch_dims > 0:
        all_indices = _ivy.concat((validity_indices[..., :-2], all_indices), axis=-1)
    ...
    if with_db:
        ...
        # BS x 1 x 1 x 1
        mean_depth_min = _ivy.expand_dims(mean_depth_min, axis=-2)
        mean_depth_range = _ivy.expand_dims(mean_depth_range, axis=-2)

        # BS x 1 x 1 x (1+D)
        pc_n_feat_wo_depth_min = _ivy.expand_dims(pc_n_feat_wo_depth_min, axis=-2)
        pc_n_feat_wo_depth_range = _ivy.expand_dims(pc_n_feat_wo_depth_range, axis=-2)

        # BS x 1 x 1 x (2+D) x 2
        var_threshold = _ivy.expand_dims(var_threshold, axis=-3)
        ...
        prior_wo_depth = _ivy.concat((prior[..., 0:2], prior[..., 3:]), axis=-1)
        ...
        quantized_var = _ivy.maximum(quantized_var_wo_increase * quantized_counter,
                                     _ivy.expand_dims(var_threshold[..., 0], axis=-2))
        quantized_var = _ivy.where(invalidity_mask, prior_var, quantized_var)
