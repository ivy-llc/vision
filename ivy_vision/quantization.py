...
def quantize_to_image(pixel_coords, final_image_dims, feat=None, feat_prior=None, with_db=False,
    ...
    if isinstance(var_threshold, tuple) or isinstance(var_threshold, list):
        ...
        var_threshold = ivy.concat((ones * var_threshold[0], ones * var_threshold[1]), axis=-1)
    else:
    ...
    if uniform_pixel_coords is None:
        uniform_pixel_coords =\
            ivy_svg.create_uniform_pixel_coords_image(final_image_dims, batch_shape, dev_str=dev_str)
    uniform_pixel_coords = uniform_pixel_coords[..., 0:2]

    # Extract Values #

    feat_prior = ivy.ones_like(feat) * feat_prior if isinstance(feat_prior, float) else feat_prior
    pixel_coords_var = ivy.ones_like(pixel_coords) * pixel_coords_var\
        if isinstance(pixel_coords_var, float) else pixel_coords_var
    feat_var = ivy.ones_like(feat) * feat_var if isinstance(feat_var, float) else feat_var
    pixel_coords_prior_var = ivy.ones(batch_shape + final_image_dims + [2]) * pixel_coords_prior_var\
        if isinstance(pixel_coords_prior_var, float) else pixel_coords_prior_var
    feat_prior_var = ivy.ones(batch_shape + final_image_dims + [d]) * feat_prior_var\
        if isinstance(feat_prior_var, float) else feat_prior_var

    # Quantize #

    # BS x N x 2
    quantized_pixel_coords = ivy.reshape(ivy.astype(ivy.round(pixel_coords), 'int32'), batch_shape + [-1, 2])

    # Combine #

    # BS x N x (2+D)
    pc_n_feat = ivy.reshape(ivy.concat((pixel_coords, feat), axis=-1), batch_shape + [-1, 2+d])
    pc_n_feat_var = ivy.reshape(ivy.concat((pixel_coords_var, feat_var), axis=-1), batch_shape + [-1, 2+d])
    ...
    # BS x N x 1
    var_validity_mask = \
        ivy.sum(ivy.astype(pc_n_feat_var < var_threshold[..., 1], 'int32'), axis=-1, keepdims=True) == 2+d
    bounds_validity_mask = ivy.logical_and(
        ivy.logical_and(quantized_pixel_coords[..., 0:1] >= 0, quantized_pixel_coords[..., 1:2] >= 0),
        ivy.logical_and(quantized_pixel_coords[..., 0:1] <= final_image_dims[1] - 1,
    ...
    if num_valid_indices == 0:
        return ivy.concat((uniform_pixel_coords[..., 0:2], feat_prior), axis=-1), \
    ...
    if with_db:
        ...
        if d == 1:
            ...
            # BS x N x 1+D
            pc_n_feat_wo_depth = ivy.concat((pc_n_feat[..., 0:2], pc_n_feat[..., 3:]), axis=-1)

            # find the min and max of each value

            # BS x 1 x 1+D
            pc_n_feat_wo_depth_max = ivy.max(pc_n_feat_wo_depth, axis=-2, keepdims=True) + 1
            pc_n_feat_wo_depth_min = ivy.min(pc_n_feat_wo_depth, axis=-2, keepdims=True) - 1
            ...
            # BS x N x (2+D)
            pc_n_feat = ivy.concat((pc_n_feat_wo_depth_scaled[..., 0:2], mean_depth,
                                          pc_n_feat_wo_depth_scaled[..., 2:]), axis=-1)
    ...
    pc_n_feat_var = ivy.gather_nd(pc_n_feat_var, validity_indices[..., 0:num_batch_dims + 1])

    # num_valid_indices x 2
    quantized_pixel_coords = ivy.gather_nd(quantized_pixel_coords, validity_indices[..., 0:num_batch_dims + 1])
    ...
    values_to_scatter = ivy.concat((means_to_scatter, vars_to_scatter, counter), axis=-1)
    ...
    quantized_img = ivy.scatter_nd(ivy.reshape(all_indices, [-1, num_batch_dims + 2]),
    ...
    if with_db:
        ...
        # BS x H x W x (1+D)
        quantized_mean_wo_depth_scaled = ivy.concat((quantized_mean_scaled[..., 0:2],
                                                           quantized_mean_scaled[..., 3:]), axis=-1)
        ...
        quantized_mean_wo_depth = ivy.where(invalidity_mask, prior_wo_depth, quantized_mean_wo_depth)

        # BS x H x W x (2+D)
        quantized_mean = ivy.concat((quantized_mean_wo_depth[..., 0:2], quantized_depth_mean,
                                           quantized_mean_wo_depth[..., 2:]), axis=-1)

        # BS x H x W x (2+D)
        quantized_var_normed = quantized_var_scaled - (quantized_depth_mean - mean_depth_min) / \
                               (mean_depth_range * min_depth_diff + MIN_DENOMINATOR)
        quantized_var = ivy.maximum(quantized_var_normed * var_vals_range + var_vals_min, var_threshold[..., 0])
        quantized_var = ivy.where(invalidity_mask, prior_var, quantized_var)
    else:
        # BS x H x W x (2+D)
        quantized_sum_mean_x_recip_var = quantized_img[..., 0:2 + d]
        quantized_var_wo_increase = ivy.where(invalidity_mask, prior_var,
                                               (1 / (quantized_img[..., 2 + d:2 * (2 + d)] + MIN_DENOMINATOR)))
        quantized_var = ivy.maximum(quantized_var_wo_increase * quantized_counter,
                                     ivy.expand_dims(var_threshold[..., 0], axis=-2))
        quantized_var = ivy.where(invalidity_mask, prior_var, quantized_var)
        quantized_mean = ivy.where(invalidity_mask, prior, quantized_var_wo_increase * quantized_sum_mean_x_recip_var)
