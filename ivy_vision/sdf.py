...
def sphere_signed_distances(sphere_positions, sphere_radii, query_positions):
    ...
    # BS x NS x 1 x 3
    sphere_positions = _ivy.expand_dims(sphere_positions, axis=-2)

    # BS x 1 x NP x 3
    query_positions = _ivy.expand_dims(query_positions, axis=-3)

    # BS x NS x NP x 1
    distances_to_centre = _ivy.sum((query_positions - sphere_positions) ** 2, axis=-1, keepdims=True)**0.5

    # BS x NS x NP x 1
    all_sdfs = distances_to_centre - _ivy.expand_dims(sphere_radii, axis=-2)

    # BS x NP x 1
    return _ivy.min(all_sdfs, axis=-3)
...
def cuboid_signed_distances(cuboid_ext_mats, cuboid_dims, query_positions, batch_shape=None):
    ...
    # BS x 4 x NP
    query_positions_trans_homo = _ivy.concat((query_positions_trans, ones), axis=-2)
    ...
    # BS x NC x NP x 1
    q_min_clipped = _ivy.minimum(_ivy.max(q, axis=-1, keepdims=True), 0.)
    q_max_clipped_len = _ivy.sum(q_max_clipped**2, axis=-1, keepdims=True)**0.5
    sdfs = q_max_clipped_len + q_min_clipped

    # BS x NP x 1
    return _ivy.min(sdfs, axis=-3)
