...
def pad_omni_image(image, pad_size, image_dims=None):
    ...
    # BS x PS x W x D
    top_border = _ivy.flip(_ivy.concat([top_left, top_right], -2), -3)
    ...
    # BS x PS x W x D
    bottom_border = _ivy.flip(_ivy.concat([bottom_left, bottom_right], -2), -3)

    # BS x H+2PS x W x D
    image_expanded = _ivy.concat([top_border, image, bottom_border], -3)
    ...
    return _ivy.concat([left_border, image_expanded, right_border], -2)
