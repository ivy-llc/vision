...
def test_weighted_image_smooth(dev_str, fw):
    if fw in ["numpy", "jax"]:
        ...
        pytest.skip()
    mean_ret, _ = ivy_smooth.weighted_image_smooth(ivy.array(td.mean_img), 1 / td.var_img, td.kernel_size)
...
def test_smooth_image_fom_var_image(dev_str, fw):
    if fw in ["numpy", "jax"]:
        # numpy and jax do not yet support depthwise 2d convolutions
        pytest.skip()
    mean_ret, var_ret = ivy_smooth.smooth_image_fom_var_image(ivy.array(td.mean_img), td.var_img, td.kernel_size,
                             td.kernel_scale)
