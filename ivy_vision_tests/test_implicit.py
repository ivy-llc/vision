...
class ImplicitTestData(TestData):
    def __init__(self):
        ...
        self.radial_depths = np.array(
            [[0, 1, 2, 3], [1, 1.5, 2, 2.5], [2, 4, 5, 6]], np.float32
        )
        self.inter_sample_distances = (
            self.radial_depths[..., 1:] - self.radial_depths[..., :-1]
        )
        self.features = np.array(
            [
                [[0.0, 0.5, 0.7], [1.0, 0.2, 0.3], [0.1, 0.2, 0.3]],
                [[0.3, 0.2, 0.1], [0.5, 0.7, 0.3], [0.1, 0.9, 0.7]],
                [[0.6, 0.4, 0.2], [0.9, 0.3, 0.6], [0.4, 0.8, 0.5]],
            ],
            np.float32,
        )
        self.densities = np.array(
            [[0.3, 2.4, 0.4], [0.2, 0.7, 0.4], [0.1, 0.3, 1.2]], np.float32
        )

        self.occ_probs = np.array(
            [
                [0.2591818, 0.9092821, 0.32967997],
                [0.09516257, 0.29531187, 0.18126929],
                [0.18126929, 0.2591818, 0.6988058],
            ],
            np.float32,
        )
        self.ray_term_probs = np.array(
            [
                [0.2591818, 0.6736127, 0.0221563],
                [0.09516257, 0.26720923, 0.11558241],
                [0.18126929, 0.21220009, 0.4238471],
            ],
            np.float32,
        )
        self.term_prob_feature_rendering = np.array(
            [
                [0.67582834, 0.2687447, 0.39015797],
                [0.17371163, 0.31010312, 0.1705867],
                [0.46928048, 0.47524542, 0.37549746],
            ],
            np.float32,
        )
        self.term_prob_var_rendering = np.array(
            [
                [0.18848504, 0.03828975, 0.00651178],
                [0.01524993, 0.12262806, 0.12084134],
                [0.02295334, 0.03439996, 0.01307792],
            ],
            np.float32,
        )
        ...
        self.z_vals = ivy.expand_dims(
            ivy_imp.stratified_sample(self.near, self.far, 3), axis=-1
        )
        rays_d = ivy.expand_dims(self.rays_d, axis=-2)
        rays_o = ivy.broadcast_to(
            ivy.reshape(
                self.rays_o, batch_shape + [1] * (num_ray_batch_dims + 1) + [3]
            ),
            rays_d.shape,
        )
...
def test_create_sampled_pixel_coords_image(dev_str, fw):
    if fw == "mxnet":
        ...
        pytest.skip()
    ivy.set_backend(fw)
    sampled_img = ivy_imp.create_sampled_pixel_coords_image(
        td.image_dims,
        td.samples_per_dim,
        (td.batch_size, td.num_cameras),
        normalized=False,
        randomize=False,
    )
...
def test_sample_images(dev_str, fw):
    ...
    img0, img1 = ivy_imp.sample_images(
        [ivy.array(td.pixel_coords_to_scatter)] * 2,
        32,
        (td.batch_size, td.num_cameras),
        td.image_dims,
    )
...
def test_stratified_sample(dev_str, fw):
    ...
    res = ivy_imp.stratified_sample(
        ivy.array(td.start_vals), ivy.array(td.end_vals), num
    )
...
def test_render_implicit_features_and_depth(
    ...
    if fw == "mxnet":
        ...
        pytest.skip()
    ivy.set_backend(fw)
    rgb, depth = ivy_imp.render_implicit_features_and_depth(
        td.implicit_fn,
        ivy.array(td.rays_o),
        ivy.array(td.rays_d),
        ivy.array(td.near),
        ivy.array(td.far),
        td.samples_per_ray,
        ivy.array(td.timestamps) if with_timestamps else None,
