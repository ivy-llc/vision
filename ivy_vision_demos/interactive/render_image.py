...
class Simulator(BaseSimulator):
    ...
    def __init__(self, interactive, try_use_sim):
        ...
        if self.with_pyrep:
            ...
            cam_quat_poses = [ivy.concat([pos, eul], -1) for pos, eul in zip(cam_positions, cam_quaternions)]
...
def main(interactive=True, try_use_sim=True, f=None, fw=None):
    fw = choose_random_backend() if fw is None else fw
    ivy.set_backend(fw)
    ...
    for _ in range(iterations):
        for cam in sim.cams:
            ...
            xyzs.append(xyz)
            rgbs.append(rgb)
        xyz = ivy.reshape(ivy.concat([xyzs], 1), (-1, 3))
        rgb = ivy.reshape(ivy.concat([rgbs], 1), (-1, 3))
