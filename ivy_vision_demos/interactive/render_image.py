...
class DummyCam:
    ...
    def cap(self):
        ...
        return ivy.array(
            np.load(
                os.path.join(this_dir, "ri_no_sim", "{}_depth.npy".format(self._name))
            )
        ), ivy.array(
            np.load(
                os.path.join(this_dir, "ri_no_sim", "{}_rgb.npy".format(self._name))
            )
        )
...
class Simulator(BaseSimulator):
    def __init__(self, interactive, try_use_sim):
        ...
        if self.with_pyrep:
            ...
            for i in range(0, 3):
                [
                    item.set_color([0.2, 0.2, 0.8])
                    for item in self._vision_sensor_rays[i]
                ]
            for i in range(4, 6):
            ...
            self._dining_table.set_orientation(np.array([np.pi / 2, 0.0, 0.0]))
            self._swivel_chair.set_position(np.array([0.33, 0.98, 0.46]))
            self._swivel_chair.set_orientation(np.array([0.0, 0.0, np.pi]))
            ...
            self._vision_sensor_body_0.set_orientation(
                np.array(
                    [-145.81 * np.pi / 180, -27.763 * np.pi / 180, 136.5 * np.pi / 180]
                )
            )
            ...
            self._vision_sensor_body_1.set_orientation(
                np.array(
                    [
                        -91.181 * np.pi / 180,
                        -30.478 * np.pi / 180,
            ...
            self._vision_sensor_body_2.set_orientation(
                np.array(
                    [-116.22 * np.pi / 180, 39.028 * np.pi / 180, -138.88 * np.pi / 180]
                )
            )
            ...
            self._vision_sensor_body_3.set_orientation(
                np.array(
                    [
                        -90.248 * np.pi / 180,
                        -1.2555 * np.pi / 180,
            ...
            self._default_camera.set_orientation(
                np.array([i * np.pi / 180 for i in [-134.8, -33.52, 151.26]])
            )
            ...
            persp_angles = ivy.array([60 * np.pi / 180] * 2)
            target_intrinsics = (
                ivy_vision.persp_angles_and_pp_offsets_to_intrinsics_object(
            ...
            cam_quaternions = [
                ivy.array([-0.40934521, 0.83571182, 0.35003018, -0.10724328]),
            ...
            cam_ext_mats = [
                ivy.inv(ivy_mech.make_transformation_homogeneous(iem))[..., 0:3, :]
                for iem in cam_inv_ext_mats
            ]
            self.cams = [
                DummyCam(calib_mat, inv_calib_mat, em, iem, n)
                for em, iem, n in zip(
                    cam_ext_mats, cam_inv_ext_mats[:-1], cam_names[:-1]
                )
            ]
            self.target_cam = DummyCam(
                target_calib_mat,
                target_inv_calib_mat,
                cam_ext_mats[-1],
                cam_inv_ext_mats[-1],
                cam_names[-1],
            )
    ...
    for _ in range(iterations):
        for cam in sim.cams:
            ...
            xyz = sim.depth_to_xyz(
                depth, cam.get_inv_ext_mat(), cam.inv_calib_mat, [1024] * 2
            )
            xyzs.append(xyz)
            rgbs.append(rgb)
        xyz = ivy.reshape(ivy.concat(xyzs, axis=1), (-1, 3))
        rgb = ivy.reshape(ivy.concat(rgbs, axis=1), (-1, 3))
        cam_coords = ivy_vision.world_to_cam_coords(
            ivy_mech.make_coordinates_homogeneous(ivy.expand_dims(xyz, axis=1)),
            sim.target_cam.get_ext_mat(),
        )
        ds_pix_coords = ivy_vision.cam_to_ds_pixel_coords(
            cam_coords, sim.target_cam.calib_mat
        )
        ...
        cv2.putText(
            a_img, "a", (13, 33), cv2.FONT_HERSHEY_SIMPLEX, 1.2, tuple([0] * 3), 2
        )
        ...
        cv2.putText(
            b_img, "b", (13, 33), cv2.FONT_HERSHEY_SIMPLEX, 1.2, tuple([0] * 3), 2
        )
        ...
        cv2.putText(
            c_img, "c", (13, 33), cv2.FONT_HERSHEY_SIMPLEX, 1.2, tuple([0] * 3), 2
        )
        ...
        cv2.putText(
            no_db_img, msg, (13, 33), cv2.FONT_HERSHEY_SIMPLEX, 1.2, tuple([0] * 3), 2
        )
        ...
        raw_imgs = np.concatenate(
            (np.concatenate((a_img, b_img), 1), np.concatenate((c_img, target_img), 1)),
            0,
        )
        to_concat = (
            (raw_imgs, no_db_img) if with_mxnet else (raw_imgs, no_db_img, with_db_img)
        )
